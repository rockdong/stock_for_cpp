...
std::shared_ptr<IndicatorResult> RSI::calculate(const std::vector<double>& prices) {
    validateData(prices, period_ + 1);
    
    auto result = std::make_shared<IndicatorResult>("RSI");
    ...
    if (retCode != TA_SUCCESS) {
        throw std::runtime_error("RSI 计算失败: TA-Lib 错误码 " + std::to_string(retCode));
    }
